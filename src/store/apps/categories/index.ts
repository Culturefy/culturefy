...
export const fetchCategoriesByUserId = createAsyncThunk(
    'category',
    async ({ dispatch, user }: { dispatch: Dispatch, user: string }) => {
        dispatch(slice.actions.handleStatus('pending'))
        try {
            const response = await CategoryServices.getById(user);
            dispatch(slice.actions.handleSelectPublicCategory(response.data.data))
            dispatch(slice.actions.handleStatus('success'))
            return response.data
        } catch (error: any) {
            dispatch(slice.actions.handleStatus('error'))
...
export const slice = createSlice({
    ...
    reducers: {
        ...
        handleSelectPublicCategory: (state, action) => {
